  ...
  async initializeBountyData(): Promise<BountyInitializationResult> {
    ...
    try {
      ...
      const processIssue = async (issue: any) => {
        ...
        // If no bounty found in labels, check comments (slower but more comprehensive)
        if (!bountyAmount && issue.comments > 0) {
          try {
            bountyAmount = await this.github.extractBountyFromComments(issue.repository_url, issue.number);
          } catch (error) {
            console.error(`Error checking comments for issue ${issue.number}:`, error);
          }
        }
