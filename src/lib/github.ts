...
export class GitHubAPI {
   ...
   */
  async getIssueComments(owner: string, repo: string, issueNumber: number): Promise<{ data: Array<{ id: number; body: string; user: { login: string }; created_at: string; updated_at: string }>; rateLimit: { remaining: number; reset: number } }> {
    const endpoint = `/repos/${owner}/${repo}/issues/${issueNumber}/comments`;
    const response = await this.request<Array<{ id: number; body: string; user: { login: string }; created_at: string; updated_at: string }>>(endpoint);
    return response;
  }

  /**
   * Extract bounty amount from issue comments
   */
  async extractBountyFromComments(repositoryUrl: string, issueNumber: number): Promise<string | null> {
    try {
      const [owner, repo] = extractRepoFromUrl(repositoryUrl).split('/');
      const commentsResponse = await this.getIssueComments(owner, repo, issueNumber);
      
      // Patterns to detect bounty amounts in comments
      ...
      // Check comments from newest to oldest for bounty announcements
